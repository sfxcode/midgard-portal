export function useArraySorting() {
  ...
  function byPropertiesOf<T extends object>(sortBy: Array<sortArg<T>>) {
    ...
    return function (obj1: T, obj2: T) {
      ...
      while (result === 0 && i < numberOfProperties) {
        // @ts-expect-error Typescript is not yet smart enough to infer that arg here is keyof T
  ...
  /**
     ...
     *
     * @param arr - the array to be sorted, all the same type T
     * @param sortBy - the names of the properties to sort by, in precedence order.
     *                 Prefix any name with `-` to sort it in descending order.
     */
  function sortArray<T extends object>(arr: T[], ...sortBy: Array<sortArg<T>>): Array<T> {
